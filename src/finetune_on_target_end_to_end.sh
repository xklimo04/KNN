...
cp -r $PROJECT_DIR/results_unfreezed_from_start $SCRATCHDIR/$PROJECT/
...
python -m src.finetune_on_target \
    ...
    --output_dir $PROJECT_DIR/results_finetuned_end_to_end \
    --num_beams 4 \
    --max_length 256 \
    --early_stopping \
    --target_path $SCRATCHDIR/$PROJECT/target \
    --model_dir results_unfreezed_from_start/best_model
