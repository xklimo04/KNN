...
cp -r $PROJECT_DIR/results_unfreezed_from_start_transform1 $SCRATCHDIR/$PROJECT/
...
python -m src.finetune_on_target_canvas \
    --batch_size 8 \
    --lr 1e-5 \
    --epochs 10 \
    --project_dir $SCRATCHDIR/$PROJECT \
    --output_dir $PROJECT_DIR/results_finetuned_transform1 \
    --num_beams 4 \
    --max_length 256 \
    --early_stopping \
    --target_path $SCRATCHDIR/$PROJECT/target \
    --model_dir results_unfreezed_from_start_transform1/best_model
